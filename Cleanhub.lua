...
local PlayerESP = {enabled = false, playerData = {}, conns = {}, discordText = "discord.gg/grey.cc"}
...
-- =========================================================
-- PLAYER ESP â€” highlight + name + speed above each other player's head
-- =========================================================
do
...
local function buildGui()
	...
	-- Keep legacy refs working â€” RED/REDDIM now read current THEME at access via metatables not needed,
	...
	drag(main)
	-- =========================================================
	-- SIDEBAR area (LEFT side of unified panel) â€” transparent, holds image+tabs+brand
	-- =========================================================
	local SIDEBAR_W = 195
	local MENU_GAP = 0 -- clean split like Vampire Hub; no dark filtered gap between image and content
	local CONTENT_OVERLAP = 8  -- small overlap â€” clean separation, no text covering
	...
	-- Photo inside the canvas â€” gets clipped to the rounded shape automatically.
	...
	-- Dark scrim on top of image â€” also inside canvas, gets clipped properly.
	...
	-- (no divider line â€” the content panel's own rounded border + lighter bg creates the visual separation)
	...
	-- Tab buttons container â€” slightly above center, moderate gap
	...
	miniBtn.Text="Grey.cc";miniBtn.TextColor3=THEME_ACCENT;miniBtn.Font=Enum.Font.GothamBold;miniBtn.TextSize=12
	...
	-- pageHolder fills the contentArea (no top tab bar â€” tabs are in the sidebar)
	...
	-- Map sections to pages: Speed/Combat/Steal â†’ Main; Visual â†’ Other; Interface â†’ Config
	...
	-- Tooze: unified progress bar â€” STEAL + fill + percentage live inside ONE pill (no separate chip)
	...
	progressPct.BackgroundTransparency=1;progressPct.Text="â€”";progressPct.TextColor3=Color3.fromRGB(230, 230, 230)
	...
	-- FPS Â· PING info on the right (now occupies the full right area)
	...
	progressRadLbl.BackgroundTransparency=1;progressRadLbl.Text="-- Â· --"
	...
	-- Tooze: status state machine â€” IDLE / READY (in steal range) / STEALING
	...
	-- live FPS Â· PING update â€” reuses the stats loop pattern from the main header
	task.spawn(function()
		local lastFrame = tick()
		local fpsSamples = {}; local fpsAvg = 60
		RunService.RenderStepped:Connect(function()
	...
	-- Tooze: 3 speed buttons â€” radio behavior (only one active at a time)
	...
	makeMobileButton("carry", "CARRY\nSPEED", function()
		if speedMode and not laggerToggled then
			speedMode = false  -- toggle off â†’ normal walk
		else
	...
	-- Tooze: CINEMATIC INTRO (4.0s) â€” falling moon, drifting starfield, glitch title
	if _introEnabled then
		...
		task.spawn(function()
			...
			darkBg.ZIndex = 1
			-- Vertical gradient: lighter at top (where moon is) â†’ darker at bottom
			...
			-- Drift loop â€” moves stars LEFTWARD continuously (parallax space feel)
			...
			title.BackgroundTransparency = 1; title.Text = "TOOZE"
			...
			subtitle.BackgroundTransparency = 1; subtitle.Text = "by Adapt"
...
local function loadConfigState()
	...
	task.spawn(function()
		...
		if cfg.autoSave ~= nil then V.autoSaveEnabled=cfg.autoSave end  -- legacy field â€” auto-save is now always-on
		...
		if cfg.sidebarArt and type(cfg.sidebarArt)=="string" then
			if cfg.sidebarArt == "111817612356516" or cfg.sidebarArt == "115117078011241" then cfg.sidebarArt = "82028776918457" end
